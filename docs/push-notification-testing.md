...
- [ ] **編集者アカウント**でイベントを新規作成 → 他端末で OS 通知
- [ ] イベントを更新 → 全購読端末に通知（編集者のみ送信可）
- [ ] イベントを削除 → 全購読端末に通知（編集者のみ送信可）
- [ ] 準備物リストを保存 → 全購読端末に通知（5分スロットル・ログイン済みユーザー）
- [ ] **スマホ**：画面下部バナーまたはヘッダーのベルから通知を有効化
- [ ] **iPhone**：ホーム画面に追加した PWA から起動してから有効化
