...
/**
 * Bloom（ブルーム）とは:
 *   明るいピクセルが周囲ににじんで「自発光している」ように見える後処理エフェクト。
 *   レンダリング済み画像バッファを GPU でフィルタリングするため、
 *   シーン描画コストに上乗せされる（Canvas の draw call とは別の処理）。
 *   emissive マテリアルや白に近い粒子が光る場合に特に有効。
 *   intensity を上げすぎると全体が白飛びするので「控えめ」が原則。
 */
export const POSTPROCESS = {
  /**
   * bloom の発光強度（0=なし、1=かなり強め）。
   * 粒子・幾何オブジェクトが「ふわっと光る」程度に留める。
   */
  BLOOM_INTENSITY: 0.5,

  /**
   * この輝度値以上のピクセルだけが bloom 対象になる（0〜1）。
   * 0.15 にすると emissive マテリアル（暗めの発光色）もちゃんと光る。
   * 高くするほど「本当に明るいところだけ」に絞られる。
   */
  BLOOM_LUMINANCE_THRESHOLD: 0.15,

  /**
   * 閾値のエッジをなめらかにする幅（0=ハードエッジ, 1=ゆるやか）。
   * 中間値にすることで光のにじみが自然になる。
   */
  BLOOM_LUMINANCE_SMOOTHING: 0.3,

  /**
   * glow の広がり半径（ミップマップブラー使用時の広がり具合）。
   * 大きいほど遠くまでにじむが処理コストも上がる。
   */
  BLOOM_RADIUS: 0.5,
} as const;
